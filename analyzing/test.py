...
from metrics.metrics import shanon_entropy, fmi_dct
...
def visualize_results(img, fused_front, fused_bev):
    shape = (10, 3)
    ...
    front = normalize(fused_front['front'])
    base_img = normalize(fused_front['base_image'])
    detail_img = normalize(fused_front['detail_image'])
    front_base_cloud = normalize(fused_front['base_cloud'])
    front_detail_cloud = normalize(fused_front['detail_cloud'])
    front_fused_base = normalize(fused_front['fused_base'])
    front_fused_detail = normalize(fused_front['fused_detail'])
    front_fused = fused_front['front_fused']
    ...
    bev_base_cloud = normalize(fused_bev['base_cloud'])
    bev_detail_cloud = normalize(fused_bev['detail_cloud'])
    bev_fused_base = normalize(fused_bev['fused_base'])
    bev_fused_detail = normalize(fused_bev['fused_detail'])
    bev_fused = normalize(fused_bev['bev_fused'])

    path = "content/"

    print(f"Shape = {fused_front['front_fused'].shape}")
    fused_front['front_fused'] = np.reshape(fused_front['front_fused'], (
    fused_front['front_fused'].shape[2], fused_front['front_fused'].shape[1], fused_front['front_fused'].shape[0]))
    fused_bev['bev_fused'] = np.reshape(fused_bev['bev_fused'], (
        fused_bev['bev_fused'].shape[2], fused_bev['bev_fused'].shape[1], fused_bev['bev_fused'].shape[0]))
    cv2.imwrite(f"{path}/front_fused.png", fused_front['front_fused'] * 255)
    cv2.imwrite(f"{path}/bev_fused.png", fused_bev['bev_fused'] * 255)

    bev = normalize(fused_bev['bev'])
    bev_color = normalize(fused_bev['bev_color'])

    print(f"Information count in Front: {shanon_entropy(fused_front['front'])}")
    print(f"Information count in Bev: {shanon_entropy(fused_bev['bev'])}")
    print(f"Information count in Image: {shanon_entropy(img)}")
    print(f"Fused FMI (Front): {fmi_dct(img, fused_front['front'], fused_front['front_fused'])}")
    print(f"Fused FMI (Bev): {fmi_dct(img, fused_bev['bev'], fused_bev['bev_fused'])}")
    ...
    sub = plt.subplot2grid(shape, (5, 2), colspan=1)
    sub.imshow(bev)
    sub.set_title("Bev (All channels)")
    cv2.imwrite(f"{path}/bev.png", cv2.cvtColor(255 * bev, cv2.COLOR_RGB2BGR))
    ...
    sub = plt.subplot2grid(shape, (9, 0), colspan=3)
    sub.imshow(front, cmap='gray')
    sub.set_title("Front view (All channels)")
    cv2.imwrite(f"{path}/front.png", 255 * front)
...
def test():
    ...
    visualize_results(img, fused_front, fused_bev)
