...
from analyzing.utils import image_utils
...
def estimate_density(point_cloud, radius, k: int):
    points = torch.from_numpy(point_cloud[:, 0:3].copy())
    rows = point_cloud.shape[0]
    queries = torch.randn([rows, 3])
    ...
    neighbors_index = ans.neighbors_index.reshape(rows, k)
    neighbors_distance = ans.neighbors_distance.reshape(rows, k)

    V = 4 * np.pi * radius ** 3
    density = np.array([np.size(np.where(neighbours < radius)) / V for neighbours in neighbors_distance])
    #density = []
    #for neighbors in neighbors_distance:
        #point_cnt = 0
    #    point_cnt = np.size(np.where(neighbors < radius))
        #for neighbor_distance in neighbors:
        #    if neighbor_distance < radius:
        #        point_cnt += 1
    #    density = np.append(density, point_cnt / V)
...
def np_to_o3d_cloud(np_cloud: np.array) -> o3d.t.geometry.PointCloud:
    ...
    radius = 10
    k = 100
    cloud.point.colors[:, 2] = estimate_density(np_cloud, radius, k)
...
def make_front_proj(cloud: np.array, width, height, intrinsic, extrinsic) -> np.array:
    ...
    front = np.zeros((depth.shape[0], depth.shape[1], 3), dtype=np.float32)
    front[:, :, 0] = intensity[:, :, 0]
    front[:, :, 1] = depth[:, :, 0]
    front[:, :, 2] = intensity[:, :, 2]
...
def make_bev_height_intensity(cloud_: np.array, resolution, height_, width_, roi):
    ...
    density_map[np.int_(cloud_top[:, 0]), np.int_(cloud_top[:, 1])] = estimate_density(cloud_top, 1000, 100)
    ...
    bev[:, :, 2] = density_map[:height_, :width_]
    bev = np.flip(bev, axis=0)
    bev = np.flip(bev, axis=1)
